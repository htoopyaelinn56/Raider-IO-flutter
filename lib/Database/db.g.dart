    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Player` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `name` TEXT NOT NULL, `cls` TEXT NOT NULL, `spec` TEXT NOT NULL, `io` REAL NOT NULL)');
...
class _$PlayerDao extends PlayerDao {
  ...
  @override
  Future<List<Player>> findAllPersons() async {
    return _queryAdapter.queryList('SELECT * FROM Person',
        mapper: (Map<String, Object?> row) => Player(
            id: row['id'] as int?,
            name: row['name'] as String,
            cls: row['cls'] as String,
  ...
  @override
  Future<void> insertPerson(Player person) async {
    await _playerInsertionAdapter.insert(person, OnConflictStrategy.abort);
  }
}
